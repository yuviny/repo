...
<tiles:insert definition="main.layout">
    ...
    <tiles:put name="title" type="string">
        <bean:message key="class.type.page.title"/>
    </tiles:put>
    ...
    <tiles:put name="javascript" type="string">
        ...
        //<!--
            ...
            function setAction(crud) { document.forms['manageClassTypeForm'].elements['crud'].value = crud; }
            ...
            function setFields(name,id,form)
            {
    ...
    <tiles:put name="body" type="string">
        ...
        <div>
            <h1><bean:message key="class.type.page.title"/></h1>
        </div>

        <html:form styleId="manageClassTypeForm" action="/ClassType/Manage" method="post">
            <input type="hidden" name="crud" value=""/>
            <div id="criteria">
                <c:if test="${!empty manageableForm}">
                    <table>
                        <tr>
                            <td><nobr><bean:message key="class.type.name"/> <div class="important">*</div></nobr></td>
                            <td>
...
<div id="entitySwitcher">
    <nobr>
        ...
        <select onchange="document.location=this.options[this.selectedIndex].value+'?ref_ClassType='+this.form.elements['id'].value;">
        ...
        <logic:present role="administrador">
            <option selected="selected" value="<html:rewrite page="/ClassType/Manage.do"/>"><bean:message key="class.type"/></option>
        </logic:present>
        <logic:present role="administrador">
                ...
                <c:if test="${!empty manageableForm.manageableList}">
                    <display:table name="${manageableForm.manageableList}" id="row" requestURI="${pageContext.request.requestURI}"
                        ...
                        <display:column media="html" sortable="false">
                            <nobr>
                                <input type="radio" name="_copy" onclick="enableUpdate(true);setFields('<formatting:escape language="javascript">${row.name}</formatting:escape>','<formatting:escape language="javascript">${row.id}</formatting:escape>',this.form);"/>
                                <input type="checkbox" name="selectedRows" value="${row.id}" onclick="verifyEnableDelete(this);"/>
                            </nobr>
                        </display:column>
                        <display:column media="xml csv excel pdf"
                            property="name"
                            titleKey="class.type.name"/>
                        <display:column media="html"
                            headerClass="name" paramId="name" maxLength="36"
                            sortProperty="name" sortable="true"
                            titleKey="class.type.name"><nobr><formatting:escape language="javascript,html">${row.name}</formatting:escape></nobr></display:column>
                    </display:table>
                </c:if>
            </div>
        ...
        <div id="pageHelpSection">
            <blockquote>
                <a href="" id="pageHelp" style="display:inline;" onclick="openWindow('<html:rewrite action="/ClassType/ManageHelp"/>','onlinehelp',true,false,760,540); return false;">
