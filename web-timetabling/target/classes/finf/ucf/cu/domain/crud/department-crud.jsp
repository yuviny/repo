...
<tiles:insert definition="main.layout">
    ...
    <tiles:put name="title" type="string">
        <bean:message key="department.page.title"/>
    </tiles:put>
    ...
    <tiles:put name="javascript" type="string">
        ...
        //<!--
            ...
            function setAction(crud) { document.forms['manageDepartmentForm'].elements['crud'].value = crud; }
            ...
            function setFields(name,id,teacher,form)
            {
    ...
    <tiles:put name="body" type="string">
        ...
        <div>
            <h1><bean:message key="department.page.title"/></h1>
        </div>

        <html:form styleId="manageDepartmentForm" action="/Department/Manage" method="post">
            <input type="hidden" name="crud" value=""/>
            <div id="criteria">
                <c:if test="${!empty manageableForm}">
                    <table>
                        <tr>
                            <td><nobr><bean:message key="department.name"/> <div class="important">*</div></nobr></td>
                            <td>
                        ...
                        <tr>
                            <td>
                                <logic:present role="administrador">
                                    <html:link action="/Teacher/Manage"><bean:message key="department.teacher"/></html:link>
                                </logic:present>
                                <logic:notPresent role="administrador">
                                    <bean:message key="department.teacher"/>
                                </logic:notPresent>
                            </td>
                            <td>
                                <c:choose>
...
<div id="entitySwitcher">
    <nobr>
        ...
        <select onchange="document.location=this.options[this.selectedIndex].value+'?ref_Department='+this.form.elements['id'].value;">
        <logic:present role="administrador">
        ...
        <logic:present role="administrador">
            <option selected="selected" value="<html:rewrite page="/Department/Manage.do"/>"><bean:message key="department"/></option>
        </logic:present>
        <logic:present role="administrador">
                ...
                <c:if test="${!empty manageableForm.manageableList}">
                    <display:table name="${manageableForm.manageableList}" id="row" requestURI="${pageContext.request.requestURI}"
                        ...
                        <display:column media="html" sortable="false">
                            <nobr>
                                <input type="radio" name="_copy" onclick="enableUpdate(true);setFields('<formatting:escape language="javascript">${row.name}</formatting:escape>','<formatting:escape language="javascript">${row.id}</formatting:escape>','<formatting:escape language="javascript">${row.teacher}</formatting:escape>',this.form);"/>
                        ...
                        <display:column media="html"
                            ...
                            titleKey="department.name"><nobr><formatting:escape language="javascript,html">${row.name}</formatting:escape></nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="teacherLabel"
                            titleKey="department.teacher"/>
                        <display:column media="html"
                            headerClass="teacher" paramId="teacher" maxLength="36"
                            sortProperty="teacherLabel" sortable="true"
                            titleKey="department.teacher"><nobr><formatting:escape language="javascript,html">${row.teacherLabel}</formatting:escape></nobr></display:column>
                    </display:table>
                </c:if>
            </div>
        ...
        <div id="pageHelpSection">
            <blockquote>
                <a href="" id="pageHelp" style="display:inline;" onclick="openWindow('<html:rewrite action="/Department/ManageHelp"/>','onlinehelp',true,false,760,540); return false;">
