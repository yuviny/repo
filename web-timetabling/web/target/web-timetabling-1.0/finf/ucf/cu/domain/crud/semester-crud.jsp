...
<tiles:insert definition="main.layout">
    ...
    <tiles:put name="title" type="string">
        <bean:message key="semester.page.title"/>
    </tiles:put>
    ...
    <tiles:put name="javascript" type="string">
        ...
        //<!--
            ...
            function setAction(crud) { document.forms['manageSemesterForm'].elements['crud'].value = crud; }
    ...
    <tiles:put name="body" type="string">
        ...
        <div>
            <h1><bean:message key="semester.page.title"/></h1>
        </div>

        <html:form styleId="manageSemesterForm" action="/Semester/Manage" method="post">
            <input type="hidden" name="crud" value=""/>
            <div id="criteria">
                <c:if test="${!empty manageableForm}">
                    <table>
                        <tr>
                            <td><nobr><bean:message key="semester.name"/> <div class="important">*</div></nobr></td>
                            <td>
...
<div id="entitySwitcher">
    <nobr>
        ...
        <select onchange="document.location=this.options[this.selectedIndex].value+'?ref_Semester='+this.form.elements['id'].value;">
        ...
        <logic:present role="administrador">
            <option selected="selected" value="<html:rewrite page="/Semester/Manage.do"/>"><bean:message key="semester"/></option>
        </logic:present>
        <logic:present role="administrador">
                ...
                <c:if test="${!empty manageableForm.manageableList}">
                    <display:table name="${manageableForm.manageableList}" id="row" requestURI="${pageContext.request.requestURI}"
                        ...
                        <display:column media="xml csv excel pdf"
                            property="name"
                            titleKey="semester.name"/>
                        <display:column media="html"
                            headerClass="name" paramId="name" maxLength="36"
                            sortProperty="name" sortable="true"
                            titleKey="semester.name"><nobr><formatting:escape language="javascript,html">${row.name}</formatting:escape></nobr></display:column>
                    </display:table>
                </c:if>
            </div>
        ...
        <div id="pageHelpSection">
            <blockquote>
                <a href="" id="pageHelp" style="display:inline;" onclick="openWindow('<html:rewrite action="/Semester/ManageHelp"/>','onlinehelp',true,false,760,540); return false;">
