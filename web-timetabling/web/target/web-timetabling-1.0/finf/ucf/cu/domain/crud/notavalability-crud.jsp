...
<tiles:insert definition="main.layout">
    ...
    <tiles:put name="title" type="string">
        <bean:message key="not.avalability.page.title"/>
    </tiles:put>
    ...
    <tiles:put name="javascript" type="string">
        ...
        //<!--
            ...
            function setAction(crud) { document.forms['manageNotAvalabilityForm'].elements['crud'].value = crud; }
            ...
            function setFields(date,not_time,id,teacherId,groupId,subjectId,form)
            {
                form.elements["dateAsString"].value = date;
                setSelect(false,form,"not_time",not_time);
    ...
    <tiles:put name="body" type="string">
        ...
        <div>
            <h1><bean:message key="not.avalability.page.title"/></h1>
        </div>

        <html:form styleId="manageNotAvalabilityForm" action="/NotAvalability/Manage" method="post">
            <input type="hidden" name="crud" value=""/>
            <div id="criteria">
                <c:if test="${!empty manageableForm}">
                    <table>
                        <tr>
                            <td><bean:message key="not.avalability.date"/></td>
                            <td>
                        ...
                        <tr>
                            <td><bean:message key="not.avalability.not.time"/></td>
                            <td>
                                <html:select name="manageableForm" property="not_time">
                    ...
                    <html:hidden name="manageableForm" property="id"/>
                        <tr>
                            <td>
                                <logic:present role="administrador">
                                    <html:link action="/Teacher/Manage"><bean:message key="not.avalability.teacher.id"/></html:link>
                                </logic:present>
                                <logic:notPresent role="administrador">
                                    <bean:message key="not.avalability.teacher.id"/>
                                </logic:notPresent>
                            </td>
                            <td>
                                <c:choose>
                        ...
                        <tr>
                            <td>
                                <logic:present role="administrador">
                                    <html:link action="/Group/Manage"><bean:message key="not.avalability.group.id"/></html:link>
                                </logic:present>
                                <logic:notPresent role="administrador">
                                    <bean:message key="not.avalability.group.id"/>
                                </logic:notPresent>
                            </td>
                            <td>
                                <c:choose>
                        ...
                        <tr>
                            <td>
                                <logic:present role="administrador">
                                    <html:link action="/Subject/Manage"><bean:message key="not.avalability.subject.id"/></html:link>
                                </logic:present>
                                <logic:notPresent role="administrador">
                                    <bean:message key="not.avalability.subject.id"/>
                                </logic:notPresent>
                            </td>
                            <td>
                                <c:choose>
...
<div id="entitySwitcher">
    <nobr>
        ...
        <select onchange="document.location=this.options[this.selectedIndex].value+'?ref_NotAvalability='+this.form.elements['id'].value;">
        <logic:present role="administrador">
        ...
        <logic:present role="administrador">
            <option selected="selected" value="<html:rewrite page="/NotAvalability/Manage.do"/>"><bean:message key="not.avalability"/></option>
        </logic:present>
        <logic:present role="administrador">
                ...
                <c:if test="${!empty manageableForm.manageableList}">
                    <display:table name="${manageableForm.manageableList}" id="row" requestURI="${pageContext.request.requestURI}"
                        ...
                        <display:column media="html" sortable="false">
                            <fmt:formatDate var="dateFormatted" value="${row.date}" pattern="dd/MM/yyyy"/>
                            <nobr>
                                <input type="radio" name="_copy" onclick="enableUpdate(true);setFields('<formatting:escape language="javascript">${dateFormatted}</formatting:escape>','<formatting:escape language="javascript">${row.not_time}</formatting:escape>','<formatting:escape language="javascript">${row.id}</formatting:escape>','<formatting:escape language="javascript">${row.teacherId}</formatting:escape>','<formatting:escape language="javascript">${row.groupId}</formatting:escape>','<formatting:escape language="javascript">${row.subjectId}</formatting:escape>',this.form);"/>
                                <input type="checkbox" name="selectedRows" value="${row.id}" onclick="verifyEnableDelete(this);"/>
                            </nobr>
                        </display:column>
                        <display:column media="xml csv excel pdf" titleKey="not.avalability.date">${dateFormatted}</display:column>
                        <display:column media="html"
                            headerClass="date" paramId="date" maxLength="36"
                            sortProperty="date" sortable="true"
                            titleKey="not.avalability.date"><nobr>${dateFormatted}</nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="not_time"
                            titleKey="not.avalability.not.time"/>
                        <display:column media="html"
                            headerClass="not_time" paramId="not_time" maxLength="36"
                            sortProperty="not_time" sortable="true"
                            titleKey="not.avalability.not.time"><nobr><formatting:escape language="javascript,html">${row.not_time}</formatting:escape></nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="teacherIdLabel"
                            titleKey="not.avalability.teacher.id"/>
                        <display:column media="html"
                            headerClass="teacherId" paramId="teacherId" maxLength="36"
                            sortProperty="teacherIdLabel" sortable="true"
                            titleKey="not.avalability.teacher.id"><nobr><formatting:escape language="javascript,html">${row.teacherIdLabel}</formatting:escape></nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="groupIdLabel"
                            titleKey="not.avalability.group.id"/>
                        <display:column media="html"
                            headerClass="groupId" paramId="groupId" maxLength="36"
                            sortProperty="groupIdLabel" sortable="true"
                            titleKey="not.avalability.group.id"><nobr><formatting:escape language="javascript,html">${row.groupIdLabel}</formatting:escape></nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="subjectIdLabel"
                            titleKey="not.avalability.subject.id"/>
                        <display:column media="html"
                            headerClass="subjectId" paramId="subjectId" maxLength="36"
                            sortProperty="subjectIdLabel" sortable="true"
                            titleKey="not.avalability.subject.id"><nobr><formatting:escape language="javascript,html">${row.subjectIdLabel}</formatting:escape></nobr></display:column>
                    </display:table>
                </c:if>
            </div>
        ...
        <div id="pageHelpSection">
            <blockquote>
                <a href="" id="pageHelp" style="display:inline;" onclick="openWindow('<html:rewrite action="/NotAvalability/ManageHelp"/>','onlinehelp',true,false,760,540); return false;">
