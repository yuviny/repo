...
<tiles:insert definition="main.layout">
    ...
    <tiles:put name="title" type="string">
        <bean:message key="subject.page.title"/>
    </tiles:put>
    ...
    <tiles:put name="javascript" type="string">
        ...
        //<!--
            ...
            function setAction(crud) { document.forms['manageSubjectForm'].elements['crud'].value = crud; }
            ...
            function setFields(name,abbreviation,id,form)
            {
    ...
    <tiles:put name="body" type="string">
        ...
        <div>
            <h1><bean:message key="subject.page.title"/></h1>
        </div>

        <html:form styleId="manageSubjectForm" action="/Subject/Manage" method="post">
            <input type="hidden" name="crud" value=""/>
            <div id="criteria">
                <c:if test="${!empty manageableForm}">
                    <table>
                        <tr>
                            <td><nobr><bean:message key="subject.name"/> <div class="important">*</div></nobr></td>
                            <td>
                                <html:text name="manageableForm" property="name" styleClass="criteriaField" styleId="name"/>
                            </td>
                        </tr>
                        <tr>
                            <td><nobr><bean:message key="subject.abbreviation"/> <div class="important">*</div></nobr></td>
                            <td>
...
<div id="entitySwitcher">
    <nobr>
        ...
        <select onchange="document.location=this.options[this.selectedIndex].value+'?ref_Subject='+this.form.elements['id'].value;">
        ...
        <logic:present role="administrador">
            <option selected="selected" value="<html:rewrite page="/Subject/Manage.do"/>"><bean:message key="subject"/></option>
        </logic:present>
        <logic:present role="administrador">
                ...
                <c:if test="${!empty manageableForm.manageableList}">
                    <display:table name="${manageableForm.manageableList}" id="row" requestURI="${pageContext.request.requestURI}"
                        ...
                        <display:column media="html" sortable="false">
                            <nobr>
                                <input type="radio" name="_copy" onclick="enableUpdate(true);setFields('<formatting:escape language="javascript">${row.name}</formatting:escape>','<formatting:escape language="javascript">${row.abbreviation}</formatting:escape>','<formatting:escape language="javascript">${row.id}</formatting:escape>',this.form);"/>
                                <input type="checkbox" name="selectedRows" value="${row.id}" onclick="verifyEnableDelete(this);"/>
                            </nobr>
                        </display:column>
                        <display:column media="xml csv excel pdf"
                            property="name"
                            titleKey="subject.name"/>
                        <display:column media="html"
                            headerClass="name" paramId="name" maxLength="36"
                            sortProperty="name" sortable="true"
                            titleKey="subject.name"><nobr><formatting:escape language="javascript,html">${row.name}</formatting:escape></nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="abbreviation"
                            titleKey="subject.abbreviation"/>
                        <display:column media="html"
                            headerClass="abbreviation" paramId="abbreviation" maxLength="36"
                            sortProperty="abbreviation" sortable="true"
                            titleKey="subject.abbreviation"><nobr><formatting:escape language="javascript,html">${row.abbreviation}</formatting:escape></nobr></display:column>
                    </display:table>
                </c:if>
            </div>
        ...
        <div id="pageHelpSection">
            <blockquote>
                <a href="" id="pageHelp" style="display:inline;" onclick="openWindow('<html:rewrite action="/Subject/ManageHelp"/>','onlinehelp',true,false,760,540); return false;">
